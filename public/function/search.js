...
const whatchType = document.getElementsByClassName("whatch");
...
async function getGenres() {
    const response1 = await fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${tmdbKey}&language=en-US`, options);
    const data1 = await response1.json();
    const response2 = await fetch(`https://api.themoviedb.org/3/genre/tv/list?api_key=${tmdbKey}&language=en-US`, options);
...
document.getElementById("searchInput").addEventListener("input", (event) => {
    ...
    checkInput();
    }, 1000); // Aspetta 300ms prima di chiamare la funzione
});
...
document.getElementById("searchInput").addEventListener("focusout", (event) => {
    setTimeout(() => {
        document.getElementById("searchSuggestions").scrollTop = 0; // Resetta lo scroll della lista dei suggerimenti
        document.getElementById("searchSuggestions").classList.remove("active");
        document.getElementById("search-loader").style.display = "none"; // Nascondi il loader
        document.getElementById("suggestionsList").innerHTML = ""; // Pulisce la lista dei suggerimenti
    }, 300); // Aspetta 300ms prima di pulire la lista
});
        ...
        // getMovieDataTheMovieDb(query).then((data) => {
            ...
            if (!data || data.length === 0) {
                if (document.getElementById("suggestionsList").children.length > 0) {
                    document.getElementById("suggestionsList").innerHTML = ""; // Pulisce la lista dei suggerimenti se non ci sono risultati
                }
                const noResultsItem = document.createElement("li");
                noResultsItem.textContent = "Nessun risultato trovato";
                noResultsItem.className = "no-results";
                document.getElementById("suggestionsList").appendChild(noResultsItem);
                document.getElementById("searchSuggestions").classList.add("active");
                return; // Esce dalla funzione se non ci sono risultati
            }
            if (data && serchFocus === document.activeElement) { // Controlla se ci sono risultati e se l'input è ancora attivo
                data.forEach((movie) => {
                    const listItem = document.createElement("li");
                    const liText = document.createElement("h3");
                    const img = document.createElement("img");
                    if(movie.image !== null)img.src = `https://image.tmdb.org/t/p/w500${movie.image}`;
                    if(movie.image === null) img.src = "/images/placeholder-image.png"; // Placeholder per le immagini non disponibili
                    img.alt = movie.title;
                    ...
                    document.getElementById("searchSuggestions").classList.add("active");
                    document.getElementById("searchSuggestions").scrollTop = 0; // Resetta lo scroll della lista dei suggerimenti quando vengono aggiunti nuovi elementi
                });
            }
            }, 200); // Simula un ritardo di 1 secondo per mostrare il loader
         });
    } 
    else {
        document.getElementById("search-loader").style.display = "none"; // Nascondi il loader se la query è troppo corta
        document.getElementById("suggestionsList").innerHTML = ""; // Pulisce la lista dei suggerimenti
        document.getElementById("searchSuggestions").classList.remove("active");
    }
}
...
function updatePageContent(movie) {
    ...
    whatchType[0].classList.remove("no-display");
    ...
    if (movie.image == null || movie.image === "") {
        image[0].src = "/images/placeholder-image.png"; // Placeholder per le immagini non disponibili
        image[1].src = "/images/placeholder-image.png"; // Placeholder per le immagini non disponibili
    }
    ...
    if (movie.image == null || movie.image === "") {
        formImage.value = "/images/placeholder-image.png"; // Placeholder per le immagini non disponibili
    }
...
async function getMovieDataTheMovieDb(title) {
    ...
    if (data.length > 0) {
        ...
        for (let i = 0; i < data.length; i++) {
            if (i < 60) { // Limita a 60 risultati per evitare troppi dati
            const genreNames = [];
            if (data[i].genre_ids) {
...
async function fetchAllResultsTv(title) {
    ...
    do {
        ...
        page++;
    } while (page <= totalPages && page <= 15); // Limita a 5 pagine per evitare richieste eccessive
...
async function fetchAllResultsMovie(title) {
    ...
    do {
        ...
        totalPages = data.total_pages;
        page++;
    } while (page <= totalPages && page <= 20); // Limita a 5 pagine per evitare richieste eccessive
    // Ordina per popolarità (dal più popolare al meno popolare)
    allResults.sort((a, b) => b.popularity - a.popularity);
    return allResults;
}
